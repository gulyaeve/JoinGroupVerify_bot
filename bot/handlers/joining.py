...
async def ban_user(member_id: int, bot: Bot, group_id: int, user_fullname: str):
    ...
    try:
        ...
        logging.warning(e)
        await notify_admins(
            text=f"{user_fullname} скорее всего остался в черном списке группы. {e}",
            bot=bot,
        )
...
async def kick(
    ...
    if str(member_id) not in ver and user_in_group.status != "left":
        ...
        await notify_admins(
            text=f"{user_fullname} ответил правильно и прошел верификацию.", bot=bot
        )
...
async def new_member(event: ChatMemberUpdated, bot: Bot):
    ...
    await notify_admins(
        text=f"{event.new_chat_member.user.full_name} {event.new_chat_member.user.username} начал верификацию в группе {event.chat.full_name}. {event.invite_link=}",
        bot=bot,
    )
...
router.chat_member.register(
    new_member, ChatMemberUpdatedFilter(member_status_changed=JOIN_TRANSITION)
)
router.callback_query.register(
    not_kick, lambda callback: callback.data == str(callback.from_user.id)
)
